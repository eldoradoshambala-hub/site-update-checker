...
- 巡回は **GitHub Actions** が 1日1回（JST 12:00）実行します
- 画面は **静的サイト**（HTML + CSS + JS）なので、サーバーの用意や維持費は不要です
- 新着はタイトル付きで一覧され、**クリックするとその記事に直接飛べます**
- 自動実行が来ていなくても、画面の **「手動更新」ボタン**から GitHub Actions を直接実行できます
...
sites.yml            監視したいサイトのURL一覧（自分で編集する）
    ↓
GitHub Actions       1日1回（自動）、または画面の「手動更新」ボタン（随時）
    ↓
...
.github/workflows/
  crawl.yml                 1日1回の巡回と自動コミット（手動実行にも対応）
  test.yml                  push時のテスト
```

## 自動実行が来ないとき（手動更新）

GitHub の `schedule` トリガーは発火を保証しません。公式ドキュメントにも
「高負荷時は遅延する」と明記されており、実際にこのリポジトリでも設定直後に
6時間以上遅延し、その後2回分が丸ごと発火しなかったことがあります。

そのため画面に **「手動更新」ボタン**を置いています。押すと GitHub の
Actions ページ（「巡回」ワークフロー）が新しいタブで開くので、
そこで **Run workflow** を押せば数分で反映されます。ページ側から直接
巡回を起動できないのは、それには GitHub のトークンを画面に埋め込む必要が
あり、公開ページでは安全に扱えないためです。
